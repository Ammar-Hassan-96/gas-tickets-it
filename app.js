...
function doLogout() {
  ...
  // مسح last_seen عند تسجيل الخروج عشان ما يظهرش كـ "متصل"
  if (S.user?.id) {
    sbFetch(`/users?id=eq.${S.user.id}`, {
      method: 'PATCH',
...
async function bootApp() {
  ...
  // 1) refresh الـ JWT لو قرب ينتهي
  // 2) تحديث last_seen في DB عشان تشتغل الجلسات النشطة في الـ Dashboard
  if (S._heartbeat) clearInterval(S._heartbeat);

  // تحديث فوري عند بدء الجلسة
  if (S.user?.id) {
    sbFetch(`/users?id=eq.${S.user.id}`, {
      method: 'PATCH',
      headers: { 'Prefer': 'return=minimal' },
