...
export default async (req) => {
  ...
  // ── RESOLVE USERNAME → EMAIL (server-side only, no public exposure) ──
  // بيحول username لـ email عشان Supabase Auth يستخدمه
  // لا يكشف معلومات عن وجود المستخدم (نفس الـ error لو username غلط أو صح)
  if (action === "resolve_username") {
    ...
    try {
      ...
      if (!users?.length || !users[0].email) {
        // نأخر الرد قليلاً عشان نمنع timing attacks
  ...
  // ── LOGIN (legacy — kept for backward compat) ─────────
  if (action === "login") {
  ...
  // ── CREATE USER PROFILE (fallback لو الـ Trigger ما شتغلش) ─
  if (action === "create_user_profile") {
    ...
    if (!allowed) return Response.json({ error: "ليس لديك صلاحية" }, { status: 403 });
    ...
    try {
      ...
      // لو duplicate — الـ Trigger سبقنا وعمل الصف
      ...
      return Response.json({ error: "فشل إنشاء الملف الشخصي: " + e.message }, { status: 500 });
    }
  }
